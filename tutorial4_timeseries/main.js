...
*/
const formatMillions = (num) => d3.format(".2s")(num).replace(/M/, 'M')
...
function init() {
    ...
    const selectElement = d3.select("#dropdown").on("change", function () {
        console.log("new selected apt type is", this.value);
...
function draw() {
    ...
    if (state.selectedType !== null) {
        filteredData = state.data.filter(d => d.apt_type === state.selectedType);
    }
    ...
    d3.select
        ...
        .transition()
        .duration(1000)
        .call(yAxis); // this updates the yAxis' scale to be our newly updated one
